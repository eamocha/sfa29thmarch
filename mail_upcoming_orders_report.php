...
?><form action="<?php $_SERVER['PHP_SELF']?>" method="post"><table ><tr><td width="54">Select Date </td>
                       ...
                       <td width="177"><input type="text" class="form-control dpd2" name="date2" value="<?php echo $date2?>"></td>
                       <td width="114"><button class="btn btn-small btn-success" type="submit" name="submit" >Fetch Info</button></td></tr></table></form> </div>
                     <!--start -->     <?php ob_start?>  <div>Orders Between  <b>  <?php echo date('D dS, M Y', strtotime($date));?></b> and <b> <?php echo date('D dS, M Y', strtotime($date2));?></b></div>     
                   <section id="unseen">
                   <?php $results=$mysqli->query("SELECT count(*) as t_records FROM `tbl_orders` where date(`date_due`)>date(now()) and status=0")or die('error');
...
$(document).ready(function() {
	...
	var dt2=<?php echo strtotime($date2)?>;
	loading_data('#results',"data.php?dt="+dt+'&dt2='+dt2+'&mode=upcoming_orders',track_load,loading,total_groups,total,pergroup);
                            ...
                            <table class="table table-bordered table-striped table-condensed">
                              ...
                              <tr>
                                  ...
                                  <th colspan="3" >Somersby</th>
                                   <th colspan="3">Tuborg</th>
                                  <th rowspan="2">Date Due</th>
                              </tr>
                              <tr>
                                <th>330ml</th>
                                <th>330ml Can</th>
  ...
  $send_to_q=mysql_query("SELECT * FROM `tbl_users` WHERE `role`>1")or die(mysql_error()); while($result=mysql_fetch_array($send_to_q)){
						  $to=$result['email']; $title='Pre-Orders ( Upcoming orders ): '.$date;
