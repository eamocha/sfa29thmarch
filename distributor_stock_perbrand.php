          ...
          <section class="wrapper">
              ...
              <div class="row" id="pdfthis">
                  ...
                  <form  method="post"><table  ><tr><td>Region </td><td> <select  class="boundary_filters form-control" name="region" id="region"><option value="-1">Select Region</option><?php echo region_selection();?></select></td><td>Area </td><td><select class="boundary_filters  form-control" name="area" id="area"> <option value="-1">Select area</option> <?php echo area_selection();?> </select></td><td> Select Date </td><td><input type="text" class="form-control dpd1" name="date" id="date" value="<?php echo $date?>"></td><td><span class="btn btn-small btn-success"  id="submit" name="submit" >Search</span></td></tr></table></form> </div> </div></span>
                     <!--start -->      <div>Summary as at  <b> <?php  echo date('D dS, M Y', strtotime($date));?></b></div>     
                   <section id="unseen"><table width="100%" class="table table-bordered table-striped table-condensed" id="tblExport">
                              <thead>
                              <tr>
                                  <th >No.</th>
                                  <th >KD</th>
                                  <?php $variants_array=array(); $skusq=$mysqli->query("SELECT distinct(variant) FROM `tbl_products` WHERE status=0 order by variant Asc ")or die($mysqli->error()); while($sku_row=mysqli_fetch_array($skusq)) { $variants_array[]=$sku_row["variant"]?>
								  <th class="rotate45"><div><span><?php echo $sku_row['variant']?></span></div></th><?php }?>
                                 
                              </tr>
                              </thead>
                              <tbody id="resultsBody">
                                <tr><td colspan="<?php echo count($variants_array)+2?>">Loading Please wait..... This may take upto 5 minutes<img src="images/37.gif"></td></tr>
 ...
 <script src="assets/js/bootstrap.min.js"></script>
	...
	$(document).ready(function(e) {
		...
		$("#submit").click(function(e) {
			$("#resultsBody").empty();
			$("#resultsBody").append('<tr><td id="loading" colspan="<?php echo count($variants_array)+2?>">Loading Please wait..... This may take upto 5 minutes<img src="images/37.gif"></td></tr>');
            var region=$("#region").val();
			var area=$("#area").val();
			var date=$("#date").val();
			  			
			var url="report_sections/stock.php?mode=distributor_stock_perBrand&region="+region+"&area="+area+"&date="+date;
	$("#resultsBody").load(url);
        });///default load
		
		
		  	var url="report_sections/stock.php?mode=distributor_stock_perBrand&date="+date;
	$("#resultsBody").load(url);    
    });
