              ...
              <div class="row" id="pdfthis">
                  <div class="col-lg-12">
                      ...
                      <div class="border-head" >
						...
						$date2=date('Y-m-d');?>
                         <h3 class="float_left">Distributor stocks since <?php echo date('d.m.Y',strtotime("-2 days"));?><span  id="img_icons"><img src="assets/img/pdf_icon.gif" onClick="javascript:demoFromHTML()" /> <img onclick="javascript:printDiv('pdfthis')" src="assets/img/print_icon.gif" ><img id="btnExport" src="assets/img/excel_icon.gif"> <img onClick="window.location='inbox.php'" src="assets/img/email_icon.gif"></span> </h3> <div>
       
    </div>
                      </div>
                    <div  style="padding-bottom:20px" class="float_left">
                     <?php ob_start(); $exc_empties =" product_id NOT IN $EMPTIES";
					 $where="status=0  AND area_id NOT IN(8,14,23)";
if(isset($_REQUEST['submit']))
{ $area_id=$_REQUEST['area'];
    $region_id = $_REQUEST['region']; 
	$where=" status=0 and region_id=$region_id and area_id=$area_id";
...
?><form action="<?php $_SERVER['PHP_SELF']?>" method="post"><table  ><tr><td>From </td><td> <select  class="form-control" name="region"><?php echo region_selection();?></select></td><td><select class="form-control" name="area" id="area"> <?php echo area_selection();?> </select></td><td> </td><td><button class="btn btn-small btn-success" type="submit" name="submit" >Search</button></td><td> </td></tr></table></form> </div>
                     <!--start -->      <div><p> Please click<a href="http://almasisfa.com/excel_distributor_stock.php"> here </a>to download an excel sheet showing Distributor Stocks for this month- MTD </p></div>     </span>
                   <section id="unseen">
   ...
   <th>#</th> <th>DISTRIBUTOR</th>
    ...
    <th ><?php echo date('d.m.Y',strtotime($jana));?></th>
      <th ><?php echo date('d.m.Y',strtotime($today));?></th>
		...
		$arq=mysqli_query($mysqli,"SELECT area_name,area_id,arm_incharge FROM tbl_areas WHERE $where order by region_id") or die(mysqli_error($mysqli));
							  while($ar=mysqli_fetch_array($arq)){ $d=1;
								...
								while($dr=mysqli_fetch_array($dq)){
								 ...
								 $ttjuzi+= $sumjuzi=sum_columns("tbl_distributor_stock_levels","qty","distributor_id=$did and  date(date_taken)='$juzi' and $exc_empties"); 
								   $ttjana+=$sumjana=sum_columns("tbl_distributor_stock_levels","qty","distributor_id=$did and  date(date_taken)='$jana' and  $exc_empties");  
								 $ttleo+= $sumleo=sum_columns("tbl_distributor_stock_levels","qty","distributor_id=$did and  date(date_taken)='$today' and $exc_empties");
								   ?>
                              <tr>
								 ...
								 <?php $d++;} 
		?> <tr style="background-color:#F00 !important; font-weight:bold; color:#FFF; text-transform:uppercase;"><td colspan="2"><?php echo $ar['area_name']?></td><td colspan="6"><?php echo getColumnName("tbl_users","full_name","user_id=".$ar['arm_incharge'])?></td><td><?php echo $ttjuzi?></td> <td><?php echo $ttjana?></td> <td><?php echo $ttleo?></td> </tr>
