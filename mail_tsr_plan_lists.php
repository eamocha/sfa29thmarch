              ...
              <div class="row" id="pdfthis">
                  <div class="col-lg-9 main-chart">
                      ...
                      <div class="border-head" >
                        <?php  include('submenu.php'); $date=date('Y-m-d'); $date2=date('Y-m-d');  $date=date("Y-m-d",strtotime($date)-86400);?>
                         <h3 class="float_left">Daily TSR Plan list<span  id="img_icons"><img src="assets/img/pdf_icon.gif" onClick="javascript:demoFromHTML()" /> <img onclick="javascript:printDiv('pdfthis')" src="assets/img/print_icon.gif" ><img id="btnExport" src="assets/img/excel_icon.gif"> <img onClick="window.location='inbox.php'" src="assets/img/email_icon.gif"></span> </h3> <div>
...
?><form action="<?php $_SERVER['PHP_SELF']?>" method="post"><table  ><tr><td>From </td><td> <input type="text" class="form-control dpd1" value="<?php echo $date;?>" name="date"></td><td> </td><td> </td><td><button class="btn btn-small btn-success" type="submit" name="submit" >Search</button></td><td> </td></tr></table></form> </div>
                   ...
                   <section id="unseen">
                         ...
                         <tbody>
                         <?php $users_q=mysql_query("SELECT * FROM `tbl_users` WHERE `role`=1 and status=0")or die(mysql_error()); while($result=mysql_fetch_array($users_q)){
							 $u_id=$result['user_id'];
							 //initialize counter 
							 $i=1;
							 $query_plans=mysql_query("SELECT * FROM `tbl_route_plan` WHERE date(`startdate`)='$date' and status=0 and `assigned_to`=$u_id")or die(mysql_error()); $n=mysql_num_rows($query_plans); if($n==0){?>
                            ...
                            <td> -</td><td> -</td><td> -</td><td> -</td></tr>
							 ...
							 ?>
						 <tr><td rowspan="<?php echo $n?>"><?php echo $result['full_name']?></td>
                         <?php while($plan_rows=mysql_fetch_array($query_plans)){ $dealer_id=$plan_rows['dealer_id']; ?><td><?php echo $i?></td>
  ...
  $send_to_q=mysql_query("SELECT * FROM `tbl_users` WHERE `role`>1")or die(mysql_error()); while($result=mysql_fetch_array($send_to_q)){
						  $to=$result['email']; $title='Yesterday plan and visit details: '.$date;
