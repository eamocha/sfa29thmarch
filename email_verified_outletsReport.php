...
<h2 style="text-align:center">Status of asset/outlet verification as at <?php echo $today_constant?>
    ...
    <table cellpadding="1px" cellspacing="1px" width="90%" style="border:#666 solid 1px; "  >
                              ...
                              <?php $i=1; $outs=0;$v_outs=0;$assts=0;$v_assts=0;
							   $q=$mysqli->query("SELECT COUNT(*) as total,area_name,a.area_id FROM `tbl_dealers` a RIGHT JOIN tbl_areas b on a.area_id=b.area_id WHERE verified=1 AND type_of_class IN ('Gold','Silver','Bronze','Other') GROUP BY a.area_id,area_name ORDER BY total DESC") or die($mysqli->error);
