...
?><form action="<?php $_SERVER['PHP_SELF']?>" method="post"><table  ><tr><td>From </td><td> <input type="text" class="form-control dpd1" value="<?php $date=date("Y-m-d",strtotime(date("Y-m-d"))-86400*7); echo $d=first_day_of_week($date);?>" name="date"></td><td> </td><td> </td><td><button class="btn btn-small btn-success" type="submit" name="submit" >Search</button></td><td> </td></tr></table></form> </div>
                     ...
                     <div><?php $next_sund=date("Y-m-d",strtotime($d)+86400*7); echo $subject="WEEKLY: ". date('D dS, M Y', strtotime($d))." - ".date('D dS, M Y', strtotime($next_sund));?></b></div>     </span>
                   <section id="unseen">
                          <table class="table table-bordered table-striped table-condensed" id="tblExport">	
                              ...
                              <tbody>
                             <?php $texp_mon=0;$texp_tue=0;$texp_wed=0;$texp_thu=0;$texp_fri=0;$texp_sat=0;$texp_sun=0; 
							$t_vist_mon=0;$t_vist_tue=0;$t_vist_wed=0;$t_vist_thu=0;$t_vist_fri=0;$t_vist_sat=0;$t_vist_sun=0; $total_exp=0; $total_vsted=0;
							 $i=1; 
			$q=mysql_query("SELECT * FROM `tbl_users` WHERE `role`=1 or `role`=2")or die(mysql_error());
							while($r=mysql_fetch_array($q)){
								$uid=$r['user_id'];
								$reg_id=$r['region_id'];
								
					$texp_mon+=$mon_exp= expected_outlets_to_visit($uid,$date );
                        $texp_tue+= $tue_exp = expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400) );
                        $texp_wed+= $wed_exp= expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400*2) );;
                        $texp_thu+=$thur_exp= expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400*3) );;
                         $texp_fri+=$fri_exp= expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400*4) );;
                         $texp_sat+= $sat_exp= expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400*5) );;
                         $texp_sun+=$sun_exp=expected_outlets_to_visit($uid,date('Y-m-d',strtotime($date)+86400*6) );;
							 //visted 
							 ...
							 $total_vsted+=$tt_w_vstd=$mon_visted+$tue_visted+$wed_visted+$thur_visted+$fri_visted+$sat_visted+$sun_visted;
                               ?>
                                 ...
                                 <tr>
								   ...
								   ?></td>
                                   <td valign="top" rowspan="3" style="text-transform:capitalize"><?php echo $r['full_name'];?></td>
  ...
  $send_to_q=mysql_query("SELECT * FROM `tbl_users` WHERE `role`>1")or die(mysql_error()); while($result=mysql_fetch_array($send_to_q)){
						  $to=$result['email']; $title='Route completion report: '.$subject;
