	...
	function map_initialize()
	{
			...
			$.get("map_process.php", function (data) {
				$(data).find("marker").each(function () {
					  ...
					  create_marker(point, name, Other_details, false, false, false, "images/icons/icon_green.png");
				});
			});	
...
<body>             
<h1 class="heading">My Outlets in a Map</h1>
<div align="center">Right Click to Drop a New Marker> then add outlet details. Click on a marker to see outlet details<table width="50%"><tr><td>Total in<b>  <?php echo region_name($region_id)?></b> Region</td><td><a href="sales_clients_list.php"><?php echo total_outlets_in_region($region_id) ?></a></td><td>Mapped</td><td><?php $mp_q=mysqli_query($mysqli,"SELECT * FROM `tbl_dealers` WHERE `status`=0 and `longtitute`!=0 and `latitute`!=0 and region_id=$region_id")or die(mysqli_error($mysqli)); echo mysqli_num_rows($mp_q)?></td><td>Deleted</td><td><b><?php echo  total_deleted_in_region($region_id);?></b></td></tr></table></div>
