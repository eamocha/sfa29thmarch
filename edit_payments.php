...
<html lang="en">
  <head>
    ...
    <![endif]--> <?php require 'header.php';  $cid=$_REQUEST['cid']; $pay_id=clean($_REQUEST['pay_id']);
              ...
              <div class="row">
                <div class="col-lg-9 main-chart">
                  ...
                  <table style="background-color:#FFF" >
                             <tr><td>
                                <form class="cmxform form-horizontal" id="commentForm" method="post" action="edit_payment_exec.php?did=<?php echo $cid; ?>&pay_id=<?php echo $pay_id?>">
                                <?php $s_pay=mysql_query("SELECT * FROM `tbl_payments` WHERE `dealer_id`=$cid AND payment_id=$pay_id")or die(mysql_error());
								$payresult=mysql_fetch_array($s_pay);?>
                                <table id="add_users"><tr>
                                 ...
                                 <td>Amount</td>
                                 <td><input class="form-control " id="amount" type="text" name="amount" value="<?php echo $payresult['amount'];?>" required /></td>
                                 <td>Mode of Payment</td>
                                 <td><select class="form-control" name="modeofpayment" id="modeofpayment">
                                   <option value="<?php echo $p=$payresult['mode_of_payment'];?>"><?php mode_of_payment($p);?></option>
                                 ...
                                 <td><input type="text" value="<?php echo $payresult['Refernce_no'];?>"class=" form-control" name="ref_no" id="ref_no"></td>
                                 </tr><tr>
                                   <td>  
                                      Description</td><td colspan="7">
                                          <textarea class ="form-control " id="description" name="description" required><?php echo $payresult['details'];?></textarea>
                                 </td></tr></table>
...
<table style="background-color:#FFF" class="table table-bordered table-striped table-condensed">
                             <thead><tr>
                              <th width="17"></th>
                              <th width="149">Date of Payment</th> <th width="52">Amount</th><th width="33" class='hidden-phone'>Ref. No</th><th width="58" class='hidden-phone'>Mode of Payment</th> <th width="80" class='hidden-phone'>Details</th><th width="40">status</th><th width="52" class='hidden-phone'>Options</th></tr></thead>
							 ...
							 else{ while($row=mysql_fetch_array($pay_query)){
 ...
 <a href="edit_payments.php?cid=<?php echo $cid?>&pay_id=<?php echo $row['payment_id'];?>">Edit</a></td></tr>
								 <?php $i++; }}?>
                              </tbody>
                                </table>
